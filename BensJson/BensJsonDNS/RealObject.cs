    ...
    internal class RealObject<T>
    {
        public T Deserialize(string json)
        {
            ...
            if (type.Name.EndsWith("[]"))
            {
                ...
                int pos = ProcessJson(json, 0, newObj, false, typeof(T).IsValueType);
            }
            else
            {
                ...
                int pos = ProcessJson(json, 0, newObj, false, newObj.GetType().IsValueType);
            }
                ...
                //{
                    if (thisChar == '[')
                    {
                        if (inList && currentObject.GetType().Name == "List`1")
                        {
                            ...
                            i = ProcessJson(json, i + 1, thisItem, true, thisItem.GetType().IsValueType);

                            addMthd.Invoke(currentObject, new object[] { thisItem });
                        }
                        else if (inList && currentObject.GetType().Name.EndsWith("[]"))
                        {
                            MethodInfo[] methods = currentObject.GetType().GetMethods();
                    ...
                    else if (thisChar == '{')
                    {
                        ...
                        else if (onValue && key.ToString() != "" && currentObject.GetType().GetTypeInfo().GetDeclaredProperty(key.ToString()).PropertyType.IsClass && currentObject.GetType().GetTypeInfo().GetDeclaredProperty(key.ToString()).PropertyType.Name != "List`1")
                        {
                            PropertyInfo info = currentObject.GetType().GetTypeInfo().GetDeclaredProperty(key.ToString());
                            object o = Activator.CreateInstance(info.PropertyType);
        ...
        private void ProcessProperty(object currentObject, string key, object value, bool isByte = false)
        {
            ...
            else
            {
                ...
                if (info.PropertyType.IsEnum)
                    methods = v.GetType().GetTypeInfo().GetEnumUnderlyingType().GetMethods();
                else
                    methods = v.GetType().GetMethods();
